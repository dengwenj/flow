...
const handleClick = () => {
  const json = graph1.value?.toJSON()
  data.value = json
  console.log(json)
}

const handleDelete = () => {
  graph1.value?.removeCells(data.value.cells)
}

const handleAdd = () => {
  graph1.value?.fromJSON(data.value)
}
</script>

<template>
  <div id="container" ref="containerRef"></div>
  <button @click="handleClick">获取 json 数据</button>
  <button @click="handleDelete">删除</button>
  <button @click="handleAdd">加载</button>
</template>